...
export default function SearchCombined() {
  ...
  async function loadSearch() {
    ...
    // ------------------- Projects Query -------------------
    let projectQuery = supabase.from("projects").select("*").order("id", { ascending: false });
    if (terms.length) {
      const orConditions = terms.map(word => {
        return projectCols.map(c => `${c}.ilike.%${word}%`).join(",");
      }).join(",");
      projectQuery = projectQuery.or(orConditions);
    }
    const { data: p1 } = await projectQuery;

    // ------------------- Prospects Query -------------------
    let prospectQuery = supabase.from("prospects").select("*").order("id", { ascending: false });
    if (terms.length) {
      const orConditions = terms.map(word => {
        return prospectCols.map(c => `${c}.ilike.%${word}%`).join(",");
      }).join(",");
      prospectQuery = prospectQuery.or(orConditions);
    }
    const { data: p2 } = await prospectQuery;
